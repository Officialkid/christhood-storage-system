...
export default function GalleryLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await fetch('/api/photo/auth/login', {
        method:  'POST',
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-16">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold tracking-tight">Christhood Gallery</h1>
        <p className="mt-1 text-sm text-zinc-400">Sign in to your gallery account</p>
      </div>
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-xs text-zinc-400 mb-1" htmlFor="password">Password</label>
            <input
              id="password"
              type="password"
        ...
        <p className="text-center text-xs text-zinc-500">
          ...
          <Link href="/signup" className="text-white underline underline-offset-2 hover:no-underline">
            Sign up
          </Link>
        </p>
        ...
        <p className="text-center text-xs text-zinc-600">
          CMMS user?{' '}
          <a href="/login" className="text-zinc-400 underline underline-offset-2 hover:no-underline">
