...
export default function PublicShareViewClient({ token }: { token: string }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        setStep('pin')
        setPinError('Enter the PIN to download this file.')
        return
      }
      if (res.status === 403) {
        setStep('pin')
      ...
      window.setTimeout(() => {
        setStep(current => (current === 'downloading' ? 'ready' : current))
      }, 3000)
    }
  }
      ...
      <div className={bg}>
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-57px)] p-4">
          <div className={`${card} p-8 max-w-sm w-full text-center space-y-4`}>
            ...
            <a href="/public-share" className="inline-block text-sm text-indigo-400 hover:text-indigo-300 transition">
              Share a file -&gt;
            </a>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-57px)] p-4">
          <div className={`${card} p-8 max-w-sm w-full space-y-5`}>
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Enter the correct PIN before this file can be opened or downloaded.</p>
            </div>
        ...
        <div className={`${card} p-8 max-w-md w-full shadow-2xl space-y-5`}>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {meta?.folderPath && <p className="text-xs text-slate-500 truncate">{meta.folderPath}</p>}
              {meta?.title && <p className="text-sm text-indigo-400 font-medium truncate">{meta.title}</p>}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3.5 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50 transition shadow-lg shadow-indigo-900/30"
          >
            {step === 'downloading' ? (
              <>
